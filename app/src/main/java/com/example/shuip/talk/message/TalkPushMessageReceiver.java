...
public class TalkPushMessageReceiver extends PushMessageReceiver {
    /** TAG to Log */
    public static final String TAG = TalkPushMessageReceiver.class
            .getSimpleName();
    ...
    public void onBind(final Context context, int errorCode, String appid,
        ...
        if (errorCode == 0) {
            // 绑定成功
            mMessageHandler.bindSuccess(context,userId,channelId);
        }else{
            //绑定不成功 3 秒后重新绑定
            if(count > 0){
                new Handler().postDelayed(new Runnable() {
                    ...
                    public void run() {
                        ...
                        PushManager.startWork(context, PushConstants.LOGIN_TYPE_API_KEY,
                                MainfestUtil.getMetaValue(context, "bd_api_key"));
                    }
                },3000);
                count--;
            }
    ...
    public void onMessage(Context context, String message,
        ...
        initMessageHandler(context);
        mMessageHandler.handleMsg(context,message);
